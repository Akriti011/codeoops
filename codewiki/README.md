> **Vendored copy, patched for CodeOops.** This directory is a copy of an
> upstream CodeWiki source tree, with CodeOops-specific changes applied on
> top of the original files (not upstream commits): single-overview
> generation mode (`config.OVERVIEW_ONLY` / `OVERVIEW_ONLY_MODE`), the
> deterministic evidence extractor (`codewiki/src/be/evidence_extractor.py`,
> new file), the rewritten HLD prompt (`codewiki/src/be/prompt_template.py`'s
> `OVERVIEW_ONLY_PROMPT`), resource bounds tuned for an 8GB Apple Silicon
> host (`codewiki/src/be/llm_services.py` — request timeout, `max_retries=0`,
> a single-flight Ollama semaphore), and the ZIP-upload local-job bridge
> (`codewiki/src/fe/routes.py`'s `/api/local-job`). See the root
> `codeoops/README.md`'s "Running with Docker" section for how this is built
> and configured. Vendored rather than referenced so the `codeoops` repo is
> fully self-contained — no dependency on the original source tree's
> location on any particular machine.
