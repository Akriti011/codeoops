...
class BackgroundWorker:
    ...
    def _process_job(self, job_id: str):
        ...
        temp_repo_dir = None
        # A local-repo job points at a directory CodeOops already extracted
        # onto the shared output volume — CodeOops owns that directory's
        # lifecycle, so this worker must not delete it on cleanup.
        owns_temp_dir = True
        ...
        try:
            ...
            if job.local_repo_path:
                job.progress = "Using local repository..."
                temp_repo_dir = job.local_repo_path
                owns_temp_dir = False
                if not os.path.isdir(temp_repo_dir):
                    raise Exception(f"Local repository path not found: {temp_repo_dir}")
            else:
            ...
            # Cleanup temporary repository — never the caller's own directory
            # for a local-repo job (owns_temp_dir is False in that case).
            if owns_temp_dir and temp_repo_dir and os.path.exists(temp_repo_dir):
                try:
