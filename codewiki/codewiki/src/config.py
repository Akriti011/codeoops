...
# Repos whose core-component code fits under this many tokens skip LLM
# clustering entirely and go through single-call "whole repository" mode
# (see cluster_modules.cluster_modules). 36_369 assumes a frontier-class
# model that reliably synthesizes many files in one long, tool-calling call;
# smaller local models are more reliable when clustering instead splits the
# repo into focused, single-file-scale modules. Override via env for a
# specific deployment's model capability.
DEFAULT_MAX_TOKEN_PER_MODULE = int(os.getenv('MAX_TOKEN_PER_MODULE', '800'))
...
# When enabled, DocumentationGenerator skips module clustering and per-module
# doc generation entirely and produces exactly one overview.md via a single
# bounded, non-agentic LLM call fed by evidence_extractor's curated evidence
# (see documentation_generator.py). Off by default so the CLI and any other
# caller keep today's full multi-module wiki behavior; the web app enables it
