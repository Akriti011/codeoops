...
class EditTool:
    ...
    def create_file(self, path: Path, file_text: str):
        if not path.parent.exists():
            ...
            raise FileNotFoundError(f"Documentation parent directory does not exist: {path.parent}")
    ...
    def write_file(self, path: Path, file: str):
        """Write a file and propagate failures to the documentation agent.

        A failed Markdown write is not recoverable as a successful documentation
        operation.  Returning after only appending a tool log let PydanticAI
        complete an agent turn even though the expected artifact was absent.
        """
        try:
            ...
            logger.error("Documentation file write failed: path=%s error=%s", path, e)
            raise
...
async def str_replace_editor(
    ...
    # Compatibility for local OpenAI-compatible models (Ollama/Qwen)
    # Some models omit the required command field.
    if command is None:
    ...
    logger.info(
        "Documentation tool invocation: tool=str_replace_editor module=%s "
        "working_dir=%s command=%s path=%s",
        ctx.deps.current_module_name,
        working_dir,
        command,
        path,
    )
    ...
    if command != "view" and path.endswith(".md"):
        ...
        if not output_path.is_file():
            raise FileNotFoundError(
                "Documentation tool completed without creating the expected Markdown "
                f"file: {output_path}"
            )
        logger.info(
