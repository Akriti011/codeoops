...
def _is_local_model(model: str, base_url: Optional[str]) -> bool:
    """Recognize common self-hosted model names and endpoints for CLI guidance."""
    model_name = (model or "").lower()
    endpoint = (base_url or "").lower()
    local_markers = ("ollama", "qwen", "llama", "mistral")
    local_hosts = ("localhost", "127.0.0.1", "host.docker.internal", "::1")
    return any(marker in model_name for marker in local_markers) or any(
        host in endpoint for host in local_hosts
    )
...
@click.option(
    "--no-fallback-model",
    is_flag=True,
    help="Disable fallback-model requests (recommended when only one local model is installed)"
)
@click.option(
    "--max-tokens",
    type=int,
...
@click.option(
    ...
    type=int,
    help="Maximum tokens per module for clustering (default: 8192)"
)
@click.option(
    "--max-token-per-leaf-module",
    type=int,
    help="Maximum tokens per leaf module (default: 4096)"
)
@click.option(
    "--llm-timeout-seconds",
    type=int,
    help="Per-model response timeout in seconds (default: 180)"
)
@click.option(
    "--max-depth",
    type=int,
...
def config_set(
    ...
    $ codewiki config set --api-key ollama-local --base-url http://host.docker.internal:11434 \\
        --main-model qwen2.5:7b --cluster-model qwen2.5:7b --no-fallback-model \\
        --max-tokens 8192 --max-token-per-module 8192 --max-token-per-leaf-module 4096
    ...
    $ codewiki config set --max-tokens 8192 --max-token-per-module 8192 --max-token-per-leaf-module 4096
    ...
    try:
        ...
        if fallback_model and no_fallback_model:
            raise ConfigurationError("Use either --fallback-model or --no-fallback-model, not both")
        if not any([api_key, base_url, main_model, cluster_model, fallback_model, no_fallback_model, max_tokens, max_token_per_module, max_token_per_leaf_module, llm_timeout_seconds, max_depth, provider, aws_region, api_version, azure_deployment, use_gitignore is not None]):
        ...
        if llm_timeout_seconds is not None:
            if llm_timeout_seconds < 1:
                raise ConfigurationError("llm_timeout_seconds must be a positive integer")
            validated_data['llm_timeout_seconds'] = llm_timeout_seconds
        ...
        manager.save(
            ...
            clear_fallback_model=no_fallback_model,
            ...
            llm_timeout_seconds=validated_data.get('llm_timeout_seconds'),
        ...
        if cluster_model:
            ...
            # Local models are valid choices for local OpenAI-compatible
            # endpoints; cloud-only suggestions are not actionable there.
            if not is_top_tier_model(cluster_model) and not _is_local_model(cluster_model, base_url):
                click.secho(
        ...
        elif no_fallback_model:
            click.secho("✓ Fallback model disabled", fg="green")
        ...
        if llm_timeout_seconds:
            click.secho(f"✓ LLM timeout seconds: {llm_timeout_seconds}", fg="green")
    ...
    try:
        ...
        if output_json:
            ...
            output = {
                ...
                "fallback_model": config.fallback_model if config and config.fallback_model else None,
                "default_output": config.default_output if config else "docs",
                "max_tokens": config.max_tokens if config else 8192,
                "max_token_per_module": config.max_token_per_module if config else 8192,
                "max_token_per_leaf_module": config.max_token_per_leaf_module if config else 4096,
                "llm_timeout_seconds": config.llm_timeout_seconds if config else 180,
            ...
            if config:
                ...
                click.echo(f"  LLM Timeout Seconds:     {config.llm_timeout_seconds}")
    ...
    try:
        ...
        if caw_mode:
            ...
            try:
                ...
                if verbose:
                    click.secho("      ✓ Valid URL", fg="green")
                else:
        ...
        if caw_mode:
            ...
            if not is_top_tier_model(config.cluster_model) and not _is_local_model(
                config.cluster_model, config.base_url
            ):
                click.secho(
        ...
        if caw_mode:
            ...
            try:
                ...
                if provider == "azure-openai" or ".openai.azure.com" in base_url_lower:
                    ...
                    endpoint = normalize_openai_base_url(config.base_url)
                    client = OpenAI(
                        api_key=api_key,
                        base_url=endpoint,
                    ...
                    if is_local_model_endpoint(endpoint, config.main_model):
                        available = {entry.id for entry in models.data}
                        configured = {
                            config.main_model,
                            config.cluster_model,
                            *({config.fallback_model} if config.fallback_model else set()),
                        }
                        missing = sorted(model for model in configured if model not in available)
                        if missing:
                            raise ConfigurationError(
                                "Configured local model(s) are unavailable at "
                                f"{endpoint}: {', '.join(missing)}. "
                                "Install them with `ollama pull <model>` or disable the fallback "
                                "with `codewiki config set --no-fallback-model`."
                            )
